...
export const ActionsBox: React.FC<ActionsBoxProps> = ({ player, enemy: _, currentTurn }) => {
    ...
    const getCooldownText = (skill: Skill): string | null => {
        ...
        return cooldownRemaining ? `${Math.floor(cooldownRemaining)}` : null;
    };
                        ...
                        <div className="flex justify-between items-center">
                            <div className="">
                                ...
                                <div className="flex items-center justify-between text-xs">
                                    <span className="text-cyan-400 font-semibold">-{Math.floor(skill.manaCost)} Mp</span>
                                    {cooldownText && (
                            ...
                            <div className="flex-1">
                                ...
                                <span className="text-xs">
                                    {item.stats?.mainStat?.hp
                                        ? `+${Math.floor(item.stats.mainStat.hp)} HP`
                                        : item.stats?.mainStat?.mana
                                            ? `+${Math.floor(item.stats.mainStat.mana)} MP`
                                            : 'Consumable'
                                    }
                                </span>
                            </div>
                ...
                >
                    <div className="flex justify-between items-center">
                        <div>
                            ...
                            <span className="text-sm font-semibold text-cyan-400">+{Math.floor(playerDataManager.calcManaGain(player))} Mp</span>
                        </div>
